...
public class CreateIndexTest {
	private static final Logger log = LoggerFactory
			.getLogger(CreateIndexTest.class);
	...
	public void testCreateIndex() throws Exception {
		...
		settings = ImmutableSettings.settingsBuilder().put("cluster.name", "cluster_node").build();
		client = buildClient(settings);

		SearchResponse response = client.prepareSearch("open_market")
				.setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
				.setQuery(QueryBuilders.matchAllQuery()) // .termQuery("item_name", "Dilbert Brown")
...
//				.prepareCreate("open_market").setSettings(setting)
//				.addMapping("market", mapping).execute().actionGet();
...
//		log.debug("{}", createIndexResponse.isAcknowledged());
	...
	protected Client buildClient(Settings settings) throws Exception {
		...
		String nodes = "localhost:9300,localhost:9301,localhost:9302"; // 검색엔진
																		// node
																		// ip:port
																		// 정보
																		// 작성.
